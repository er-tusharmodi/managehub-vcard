...
<div class="col-12">
    <div class="row g-3">
        ...
        @php
            $rawVal    = is_array($gItem) ? ($gItem['product_image'] ?? '') : $gItem;
            // Extract URL from CSS background-image format: url('...') center/cover ...
            if (preg_match("/url\(['\"]?(.*?)['\"]?\)/i", $rawVal, $m)) {
                $previewUrl = $m[1];
            } else {
                $previewUrl = $rawVal; // plain URL already
            }
        @endphp
        <div class="col-md-4 col-sm-6" wire:key="mm-gallery-{{ $gi }}">
            <div class="border rounded-3 p-2" style="background:#f8fafc;">
                ...
                @else
                    <div class="mb-2 d-flex align-items-center justify-content-center rounded"
                        ...
                        <i class="mdi mdi-image-outline fs-2 text-muted"></i>
                    </div>
                @endif

                {{-- Upload --}}
                <div class="mb-2">
                    <div wire:loading wire:target="uploads.{{ $gi }}.product_image" class="text-primary small mb-1">
                        <i class="mdi mdi-loading mdi-spin me-1"></i>Uploading…
                    </div>
                    <input type="file"
                           class="form-control form-control-sm"
                           wire:model.live="uploads.{{ $gi }}.product_image"
                           accept="image/*">
                </div>
...
<div class="col-12 mt-2">
    <small class="text-muted">
        ...
        Upload an image for each slot, then click <strong>Save Changes</strong> to apply.
    </small>
</div>
