{{-- restaurant-cafe-template/payments.blade.php --}}
{{-- Table + Modal (Add/Edit) — overrides _shared/payments.blade.php --}}
@php
    $payIconDefs = [
        ...
        'wallet' => ['label' => 'Wallet', 'color' => '#e65100',
                     'svg' => '<svg viewBox="0 0 24 24" stroke-width="1.8" fill="none" stroke="currentColor" style="width:18px;height:18px;"><rect x="1" y="4" width="22" height="16" rx="2"/><circle cx="8" cy="12" r="3" fill="none"/><circle cx="14" cy="12" r="3" fill="none"/></svg>'],
            ...
            <table class="table table-sm table-hover align-middle mb-0" style="font-size:.83rem;">
                <thead>
                    <tr style="background:linear-gradient(90deg,#f0fdf4,#dcfce7);border-bottom:2px solid #86efac;">
                        ...
                        <th class="py-2 text-muted fw-semibold" style="font-size:.68rem;">Sub-label</th>
                ...
                <tbody>
                    ...
                    @php
                        ...
                        $curStroke = $row['stroke'] ?? ($payIconDefs[$curIcon]['color'] ?? '#1565c0');
                        $curName   = $row['name']   ?? '';
                        $curSub    = $row['sub']    ?? '';
                    @endphp
                    <tr wire:key="pay-row-{{ $pi }}" class="border-bottom">
                        ...
                        <td class="py-1 text-muted" style="font-size:.78rem;">{{ $curSub ?: '—' }}</td>
                        <td class="py-1 text-center">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-sm-6">
                        ...
                        <input type="text"
                               ...
                               placeholder="Cards / UPI / Cash">
                    </div>

                    {{-- Sub-label --}}
                    <div class="col-sm-6">
                        <label class="form-label small fw-semibold mb-1">Sub-label</label>
                        <input type="text"
                               class="form-control"
                               wire:model.blur="editingItem.sub"
