...
$occasionIcons = [
    'heart' => '<i class="bi bi-heart-fill ico" aria-hidden="true"></i>',
    'smile' => '<i class="bi bi-emoji-smile-fill ico" aria-hidden="true"></i>',
];

$customIcons = [
    'shield' => '<i class="bi bi-shield-check ico" aria-hidden="true"></i>',
    'group' => '<i class="bi bi-people-fill ico" aria-hidden="true"></i>',
    'delivery' => '<i class="bi bi-truck ico" aria-hidden="true"></i>',
    'star' => '<i class="bi bi-stars ico" aria-hidden="true"></i>',
];

$paymentIcons = [
    'upi' => '<i class="bi bi-phone-fill ico" aria-hidden="true"></i>',
    'bank' => '<i class="bi bi-bank2 ico" aria-hidden="true"></i>',
...
$actionData = [
    ...
    'messages' => [
        ...
        'occasionLabel' => (string) ($messages['occasionLabel'] ?? ''),
        'fallbackOccasion' => (string) ($messages['fallbackOccasion'] ?? ''),
    ...
    'promo' => [
        'enquiryText' => (string) ($promo['enquiryText'] ?? ''),
    ],
  ...
  <body>
    <div class="banner">
      <div class="banner-bg">
        <div class="banner-decoration">
          <?php foreach (a($data, 'banner.decorationIconClasses') as $iconClass): ?>
            <i class="bi <?= e($iconClass) ?> ico-xl" aria-hidden="true"></i>
          <?php endforeach; ?>
        </div>
      </div>
    ...
    <div class="profile-card">
      <div class="profile-avatar-wrap">
        <div class="profile-avatar">
          <i class="bi <?= e(v($data, 'hero.profile.avatarIconClass', 'bi-gem')) ?> ico-xl" aria-hidden="true"></i>
        </div>
      </div>
      ...
      <div class="sec-body">
        <a class="address-link" href="#" onclick="return (openMaps(), false);">
          ...
          <div class="addr-text">
            ...
            <span><?= e(v($data, 'sections.location.secondary')) ?></span><br />
            <span style="font-size:0.74rem;color:var(--pink);font-weight:600"><?= e(v($data, 'sections.location.landmark')) ?></span>
            <span class="map-btn"><i class="bi bi-geo-alt-fill ico-sm" aria-hidden="true"></i><span><?= e(v($data, 'sections.location.mapButton')) ?></span></span>
          </div>
        </a>
      </div>
    </div>
    ...
    <div class="sec">
      <div class="sec-header">
        ...
        <div class="sec-title"><?= e(v($data, 'sections.collections.title')) ?></div>
      </div>
      <div class="sec-body">
        <div class="services-grid">
          <?php foreach (a($data, 'sections.collections.items') as $item): ?>
            <?php if (!is_array($item)) { continue; } ?>
            <div class="svc-card">
              <div class="svc-img">
                <div class="svc-img-placeholder" style="background:<?= e($item['background'] ?? '') ?>">
                  <i class="bi <?= e($item['iconClass'] ?? 'bi-stars') ?> ico-xl" aria-hidden="true"></i>
                </div>
              </div>
              <div class="svc-body">
    ...
    <div class="sec">
      ...
      <div class="sec-body">
        <div class="products-grid" id="productsGrid">
          <?php $productIconClass = (string) v($data, 'sections.products.iconClass', 'bi-gem'); ?>
          <?php foreach ($products as $product): ?>
            ...
            <div class="prod-card">
              <div class="prod-img">
                <div class="prod-img-placeholder" style="background:<?= e($product['bg'] ?? '') ?>;height:100%;font-size:2rem;display:flex;align-items:center;justify-content:center"><i class="bi <?= e($productIconClass) ?> ico-xl" aria-hidden="true"></i></div>
                <?php if ($hasTag): ?>
                  <span class="prod-tag" style="background:<?= e($product['tagColor'] ?? '#d63a7a') ?>"><?= e($product['tag'] ?? '') ?></span>
                <?php endif; ?>
              </div>
              <div class="prod-body">
                <div class="prod-name"><?= e($product['name'] ?? '') ?></div>
                <div class="prod-desc"><?= e($product['desc'] ?? '') ?></div>
    ...
    <div class="sec">
      <div class="sec-header">
        ...
        <div class="sec-title"><?= e(v($data, 'sections.sizeGuide.title')) ?></div>
      </div>
      <div class="sec-body">
        <div class="size-guide">
          <?php foreach (a($data, 'sections.sizeGuide.badges') as $item): ?>
            <?php if (!is_array($item)) { continue; } ?>
            <div class="size-badge">
              <div class="size-num"><?= e($item['size'] ?? '') ?></div>
              <div class="size-label"><?= e($item['label'] ?? '') ?></div>
            </div>
          <?php endforeach; ?>
        </div>
        <div class="size-note"><?= e(v($data, 'sections.sizeGuide.note')) ?></div>
      </div>
    </div>
    ...
    <div class="sec">
      <div class="sec-header">
        ...
        <div class="sec-title"><?= e(v($data, 'sections.occasions.title')) ?></div>
      </div>
      <div class="sec-body">
        <div class="occasion-tags">
          <?php foreach (a($data, 'sections.occasions.items') as $item): ?>
            <?php
            if (!is_array($item)) { continue; }
            $iconClass = (string) ($item['iconClass'] ?? 'bi-stars');
            ?>
            <div class="occ-tag">
              <i class="bi <?= e($iconClass) ?> ico-sm" aria-hidden="true"></i>
              <?= e($item['label'] ?? '') ?>
            </div>
          <?php endforeach; ?>
        </div>
    ...
    <div class="sec">
      <div class="sec-header">
        ...
        <div class="sec-title"><?= e(v($data, 'sections.customization.title')) ?></div>
      </div>
      <div class="sec-body">
        <div class="custom-list">
          <?php foreach (a($data, 'sections.customization.items') as $item): ?>
            <?php
            if (!is_array($item)) { continue; }
            $iconKey = (string) ($item['icon'] ?? '');
            ?>
            <div class="custom-item">
              <div class="custom-icon"><span class="custom-icon-glyph"><?= $customIcons[$iconKey] ?? '' ?></span></div>
              <div>
                <div class="custom-title"><?= e($item['title'] ?? '') ?></div>
                <div class="custom-desc"><?= e($item['description'] ?? '') ?></div>
              </div>
            </div>
          <?php endforeach; ?>
        </div>
    ...
    <div class="sec">
      ...
      <div class="sec-body">
        <div class="gallery-grid">
          <?php foreach (a($data, 'sections.gallery.items') as $item): ?>
            <?php if (!is_array($item)) { continue; } ?>
            <div class="g-item"<?= (string) ($item['cardHeight'] ?? '') !== '' ? ' style="height:' . e($item['cardHeight']) . '"' : '' ?>>
              <div class="gallery-inner" style="height:<?= e($item['height'] ?? '100%') ?>;background:<?= e($item['background'] ?? '') ?>;display:flex;align-items:center;justify-content:center;font-size:2rem"><i class="bi <?= e($item['iconClass'] ?? 'bi-image-fill') ?> ico-xl" aria-hidden="true"></i></div>
            </div>
          <?php endforeach; ?>
        </div>
      </div>
    ...
    <div class="sec">
      ...
      <div class="sec-body">
        ...
        <div style="margin-top:0.75rem;background:#fff8fb;border-radius:10px;padding:0.6rem 0.85rem;font-size:0.75rem;color:var(--muted);border:1px solid var(--border);">
          <span><?= e(v($data, 'sections.hours.seasonNote.prefix')) ?></span>
          <strong style="color:var(--pink)"><?= e(v($data, 'sections.hours.seasonNote.highlight')) ?></strong>
          <span><?= e(v($data, 'sections.hours.seasonNote.suffix')) ?></span>
        </div>
    ...
    <div class="sec">
      ...
      <div class="sec-body">
        <div id="contactForm">
          ...
          <div class="bf-group">
            <label class="bf-label"><?= e(v($data, 'sections.contact.labels.occasion')) ?></label>
            <select class="bf-input" id="cOccasion">
              <?php foreach (a($data, 'sections.contact.occasionOptions') as $idx => $option): ?>
                <?php $optionText = (string) $option; ?>
                <option value="<?= $idx === 0 ? '' : e($optionText) ?>"><?= e($optionText) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="bf-group">
        ...
        <div id="contactSuccess" style="display:none;text-align:center;padding:1.8rem 1rem">
          ...
          <div style="font-size:0.98rem;font-weight:700;color:var(--text);margin-bottom:0.4rem;"><?= e(v($data, 'sections.contact.successTitle')) ?></div>
          <div style="font-size:0.8rem;color:var(--muted);margin-bottom:1rem"><?= e(v($data, 'sections.contact.successText')) ?></div>
          <button class="cf-submit" onclick="resetContact()">
            <i class="bi bi-arrow-clockwise ico" aria-hidden="true"></i>
            <span><?= e(v($data, 'sections.contact.successAction')) ?></span>
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="vcard-footer">
      <p>
        <span><?= e(v($data, 'footer.line1Prefix')) ?></span>
        <strong><?= e(v($data, 'shop.name')) ?></strong>
        <span><?= e(v($data, 'footer.line1Suffix')) ?></span>
      </p>
      <p style="margin-top:0.3rem;font-size:0.68rem">
        <span><?= e(v($data, 'footer.line2Prefix')) ?></span>
        <strong><?= e(v($data, 'footer.line2Brand')) ?></strong>
      </p>
    </div>
    ...
    <div class="float-bar">
      ...
      <div class="fab cart-fab fab-wrap" onclick="openCart()">
        ...
        <span style="font-size:0.67rem;font-weight:700;color:#b5004e"><?= e(v($data, 'floatingBar.cart')) ?></span>
      </div>
    </div>
