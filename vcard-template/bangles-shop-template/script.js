...
const openCart = () => {
    ...
    let html = picked
        .map((p) => {
            ...
            total += sub;
            return `<div class="cart-item"><div class="ci-name">${p.name}<br><small style="color:var(--muted);font-weight:400">₹${p.price}${p.per || ""}</small></div><div class="ci-qty"><button class="ci-qty-btn" onclick="changeQty(${p.id},-1);openCart()"><i class="bi bi-dash-lg ico-sm" aria-hidden="true"></i></button><span class="ci-qty-num">${state.cart[p.id]}</span><button class="ci-qty-btn" onclick="changeQty(${p.id},1);openCart()"><i class="bi bi-plus-lg ico-sm" aria-hidden="true"></i></button></div><div class="ci-price">₹${sub}</div></div>`;
        })
        .join("");
...
const sendCartWA = async () => {
    ...
    picked.forEach((p) => {
        const sub = p.price * state.cart[p.id];
        total += sub;
        message += `• ${p.name} x${state.cart[p.id]} = ₹${sub}\n`;
        orderItems.push({
            name: p.name,
            qty: state.cart[p.id],
            price: p.price,
    ...
    await sendSubmission("order", {
        source_template: state.data?.meta?.title || "bangles-shop-template",
        shop_name: shop.name || "",
        name: "",
        phone: "",
        email: "",
...
const enquireWA = (topic) => {
    ...
    const text = `Hi! Interested in: *${topic}*. Please share details.`;
    window.open(
...
const shareNative = () => {
    ...
    if (navigator.share) {
        navigator.share({ title: state.data.shop.name, url: getWebsite() });
    } else {
        copyLink();
    }
};
...
const submitContact = async () => {
    ...
    await sendSubmission("contact", {
        ...
        email: "",
        message: msg,
        items: [{ label: "occasion", value: occasion }],
    });
...
const resetContact = () => {
    ...
    const occasionSelect = document.getElementById("cOccasion");
